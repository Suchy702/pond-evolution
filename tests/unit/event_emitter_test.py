...
def test_emit_event(event_emitter):
    event_emitter.graphic_event_manager = Mock()
    graphic_event = GraphicEvent(GraphicEventType.ANIM_MOVE)
    event_emitter.emit_event(graphic_event)
    event_emitter.graphic_event_manager.add_event.assert_called_once()
...
def test_emit_events(event_emitter):
    ...
    assert event_emitter.graphic_event_manager.add_event.call_count == 10
