...
class PondViewer:
    ...
    def _calc_indices(self, p1: Position, p2: Position) -> tuple[Optional[list[int]], Optional[list[int]]]:
        x_swapped = self._swap_x(p1, p2)
        y_swapped = self._swap_y(p1, p2)
        ...
        self._restore_coordinates(p1, p2, x_swapped, y_swapped)
    ...
    @staticmethod
    def _swap_x(p1: Position, p2: Position) -> bool:
        if p1.x > p2.x:
            p1.x, p2.x = p2.x, p1.x
            return True
        return False
    ...
    @staticmethod
    def _swap_y(p1: Position, p2: Position) -> bool:
        if p1.y > p2.y:
            p1.y, p2.y = p2.y, p1.y
            return True
        return False
    ...
    @staticmethod
    def _restore_coordinates(p1: Position, p2: Position, x_swapped: bool, y_swapped: bool) -> None:
        if x_swapped:
            p1.x, p2.x = p2.x, p1.x
        if y_swapped:
            p1.y, p2.y = p2.y, p1.y
